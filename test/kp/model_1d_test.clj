(ns kp.model-1d-test
  (:require [clojure.test :refer :all]
            [kp.model-1d :refer :all]
            [kp.common :refer [clamp]]))
...
(deftest test-identical-layers-reduction
  (testing "two identical layers should reduce to single layer case"
    (let [;; Single layer: well width 0.4, barrier width 0.2, V0=8.0, period=0.6
          single-params {:a 0.6 :b 0.4 :V0 8.0 :mu 1.0}
          ;; Two identical layers: [well:0.4,V:0] [barrier:0.2,V:8]
          ;; This should be equivalent to the single layer case
          two-identical-layers [{:w 0.4 :V 0.0} {:w 0.2 :V 8.0}]
          two-layer-opts {:mu 1.0}
          test-energies [0.5 2.0 5.0 10.0 15.0]]
      
      ;; Test that D(E) values are approximately equal
      (doseq [E test-energies]
        (let [single-D (dispersion E single-params)
              {:keys [D]} (dispersion-multilayer E two-identical-layers two-layer-opts)]
          (is (< (Math/abs (- single-D D)) 1e-10)
              (str "D(E) mismatch at E=" E ": single=" single-D ", two-identical-layers=" D))))
      
      ;; Test that allowed/forbidden states match exactly
      (doseq [E test-energies]
        (let [single-allowed (allowed? E single-params)
              two-layer-D (dispersion-multilayer E two-identical-layers two-layer-opts)
              two-layer-allowed (<= (Math/abs (:D two-layer-D)) 1.0)]
          (is (= single-allowed two-layer-allowed)
              (str "Allowed state mismatch at E=" E ": single=" single-allowed ", two-identical-layers=" two-layer-allowed))))))

(deftest test-different-layer-systems
  (testing "different layer configurations produce different but valid physics"
    (let [;; Single KP parameters: a=1.0, b=0.3, V0=8.0
          single-params {:a 1.0 :b 0.3 :V0 8.0 :mu 1.0}
          ;; Different two-layer system: [well:0.7,V:0] [barrier:0.3,V:8]
          ;; This is NOT equivalent to single KP (different physical model)
          different-layers [{:w 0.7 :V 0.0} {:w 0.3 :V 8.0}]
          two-layer-opts {:mu 1.0}
          test-energies [0.5 2.0 5.0 10.0 15.0]]
      ...
      (doseq [E test-energies]
        (let [single-D (dispersion E single-params)
              {:keys [D]} (dispersion-multilayer E different-layers two-layer-opts)]
          ...
          (is (and (number? D) (not (Double/isNaN D)) (not (Double/isInfinite D)))
              (str "Different layers D(E) invalid at E=" E ": " D))))
      
      ;; Test that both methods can identify allowed/forbidden states
      (doseq [E test-energies]
        (let [single-allowed (allowed? E single-params)
              two-layer-D (dispersion-multilayer E different-layers two-layer-opts)
              two-layer-allowed (<= (Math/abs (:D two-layer-D)) 1.0)]
          ...
          (is (boolean? two-layer-allowed)
              (str "Different layers allowed? invalid at E=" E ": " two-layer-allowed))))))))
